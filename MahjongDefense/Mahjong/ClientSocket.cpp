...
bool ClientSocket::initSocket(const string& sIp, const int& iPort) {
	SOCKET lhSocket;
	...
	if (lConnect != 0) {
		cout << "lConnect = " << lConnect << "  Connect Error. Program aborted\n";
		return false;
	}
	m_sock = lhSocket;
...
void ClientSocket::sendData(const string& sData) const {
	...
	if (iLength < (int)sData.size()) { cout << "Send Error.\n"; }
}
