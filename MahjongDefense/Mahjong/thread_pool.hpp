...
public:
    ...
    void parallelize_loop(const T1& first_index, const T2& index_after_last, const F& loop, ui32 num_blocks = 0)
    {
        ...
        last_index--; /// 跑Θindex
        /// num_block程рloopちΘ碭场だ
        if (num_blocks == 0)
            num_blocks = thread_count; /// num_block箇砞thread_count(ヘ玡ノthread计秖)
        ui64 total_size = (ui64)(last_index - the_first_index + 1); /// 羆璶家览Ω计
        ui64 block_size = (ui64)(total_size / num_blocks); /// thread璶家览碭Ω
        if (block_size == 0)
        {
            block_size = 1;
